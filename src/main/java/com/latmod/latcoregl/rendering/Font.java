...
public class Font
{
    ...
    {
        if(s == null || removeFormatting(s).isEmpty())
        {
            return;
        }
        ...
        for(int i = 0; i < s.length(); i++)
        {
            ...
            if(hadCode)
            {
                ...
                if(drawAtributes)
                {
                    if(c == TextColor.BOLD.code)
                    {
                        isBold = !isBold;
                    }
                    else if(c == TextColor.ITALIC.code)
                    {
                        isItalic = !isItalic;
                    }
                    else if(c == TextColor.UNDERLINE.code)
                    {
            ...
            if(isBold)
            {
                Renderer.rect(posX + 1D, 1D, fontSize, fontSize, x1, y1, cs, cs);
            }
            posX += f;
        }
